...
def __sort_vcf__(vcf_file_path: str):
    ...
    try:
        ...
        os.system(
            f"sort -k1,1V -k2,2n {content_file_path} > {sorted_content_file_path}"
        )
        os.system(
            f'cat {header_file_path} {sorted_content_file_path} > "{vcf_file_path}"'
        )
...
def __create_unified_file__(files_lines: dict, output_path: str, skip_svtype: bool):
    ...
    with open(output_path, "w+") as output_h:
        ...
        if files_lines["sv"] is not None:
            ...
            cnv_printed = True
            output_h.write(
                "".join(filter(lambda l: __is_valid_line__(l), files_lines["cnv"]))
            )
...
def run(
    ...
    for file_type in struct_files.keys():
        ...
        if struct_files[file_type] is None:
            logging.warning(
                f'Can\'t unify file type "{file_type}". The file does not exist.'
            )
            continue
        ...
        if __is_empty__(lines):
            logging.warning(
                f'Can\'t unify file type "{file_type}". The file is empty or contains only headers.'
            )
            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Filter PacBio repeats file with pre-defined bed file and unify structural vcf files"
    )
    parser.add_argument(
        "-o",
        "--outputPath",
    ...
    parser.add_argument(
        "-modify",
        action="store_true",
        help="a flag indicating whether to modify the repeat calls or not. Add this flag when STRaglr is the repeat caller",
    )
