...
} from '../../nf-core/utils_nfschema_plugin'
include {
    paramsSummaryMap
...
} from '../../nf-core/utils_nextflow_pipeline'
...
workflow PIPELINE_INITIALISATION {
    ...
    .map {
    row -> // row is an ArrayList: [meta, bam_dir, fastq_dir, aligned_bam, methyl_bam, hpo_terms]
...
def methodsDescriptionText(mqc_methods_yaml) {
    ...
    manifest_doi.each {
        doi_ref -> temp_doi_ref += "(doi: <a href=\'https://doi.org/${doi_ref.replace("https: //doi.org/", "").replace(" ", "")}\'>${doi_ref.replace("https://doi.org/", "").replace(" ", "")}</a>), "
    }
