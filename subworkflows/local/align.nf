...
include {
    MINIMAP2_INDEX
} from '../../modules/nf-core/minimap2/index/main'
include {
    MINIMAP2_ALIGN
} from '../../modules/nf-core/minimap2/align/main'
include {
    WINNOWMAP_ALIGN
} from '../../modules/local/winnowmap/main'
...
workflow alignment_subworkflow {
    ...
    ch_align_input = ch_fastq
        ...
        .map {
        meta_sample, reads, meta_ref, index -> // Create tuple with sample meta for both reads and index
[meta_sample, reads, meta_sample, index]
    ...
    MINIMAP2_ALIGN(
        ch_align_input.map {
        meta_sample, reads, meta_index, index -> [meta_sample, reads]
        },
        ch_align_input.map {
        meta_sample, reads, meta_index, index -> [meta_index, index]  // Use sample meta for index too as otherwise runs one sample at a time
        },
        bam_format,
        bam_index_extension,
        cigar_paf_format,
