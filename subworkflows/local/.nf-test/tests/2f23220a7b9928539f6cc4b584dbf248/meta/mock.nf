...
include { ALIGN } from '/home/nmahfel/nf-training/longraredisease/subworkflows/local/align/tests/../main.nf'
...
workflow {
    ...
    //run workflow
    ALIGN.run(input.toArray())
    
    if (ALIGN.output){

        // consumes all named output channels and stores items in a json file
        ALIGN.out.getNames().each { name ->
            serializeChannel(name, ALIGN.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  
    
        // consumes all unnamed output channels and stores items in a json file
        def array = ALIGN.out as List<Object>
        def i = 0
        array.each { output ->
