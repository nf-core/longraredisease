...
def processVersionsFromYAML(yaml_file) {
    ...
    def versions = yaml.load(yaml_file).collectEntries {
    k, v -> [k.tokenize(':')[-1], v]
    }
...
def softwareVersionsToYAML(ch_versions) {
    return ch_versions.unique().map {
    version -> processVersionsFromYAML(version)
...
def paramsSummaryMultiqc(summary_params) {
    ...
    .keySet()
    .each {
    group -> def group_params = summary_params.get(group)
    ...
    if (group_params) {
        ...
        .sort()
        .each {
        param -> summary_section += "        <dt>${param}</dt><dd><samp>${group_params.get(param) ?: '<span style=\"color:#999999;\">N/A</a>'}</samp></dd>\n"
        }
        summary_section += "    </dl>\n"
    }
    }
...
//
def completionEmail(summary_params, email, email_on_fail, plaintext_email, outdir, monochrome_logs = true, multiqc_report
    = null) {
    ...
    .sort()
    .each {
    group -> summary << summary_params[group]
    }
    def misc_fields = [:]
