...
include { RTGTOOLS_FORMAT_REF                } from '../modules/local/rtgtools/format_ref/main.nf'
include { rtgtools_vcf_comparison            } from '../subworkflows/local/rtgtools_vcf_comparison.nf'
...
*/
    ...
    if (params.trio_analysis){
        RTGTOOLS_FORMAT_REF(ch_fasta).first()  // Run format_ref early to get SDF for trio comparison
    }
...
*/
    ...
    if (params.snv) {
        ...
        if (params.trio_analysis){
               ...
               // Existing SNV calling...

               ch_gvcf = call_snv.out.clair3_gvcf

               // Joint genotyping - all logic moved to subworkflow
               ...
               joint_genotype_snv(
                ...
                if (!ch_all_trio_vcfs.isEmpty()) {
                    ch_fasta_per_family = ch_all_trio_vcfs
                    .map { meta, vcf -> meta.id }  // Get family ID
                    .unique()  // Deduplicate family IDs
                    .combine(ch_fasta.map { meta, fasta -> fasta })
                    .map { family_id, fasta -> [[id: family_id], fasta] }}

                ch_fasta_per_family.view { "DEBUG: FASTA for trio comparison: $it" }

                rtgtools_vcf_comparison(
                    ch_fasta_per_family,
                    ch_trio_snv_vcf,
                ...
                ch_trio_snv_vcf = rtgtools_vcf_comparison.out.mendelian_vcf
                .map { meta, vcf -> [[id: meta.id], vcf] }
                .view { "DEBUG: Mendelian VCF: $it" }
...
if (params.sv && params.snf_output && params.trio_analysis) {
    ...
    .filter { family_id, snf_files -> snf_files.size() == 3 }
    .map { family_id, snf_files -> [[id: family_id], snf_files] }
    ...
    rtgtools_vcf_comparison(
        ...
        ch_trio_sv_vcf = rtgtools_vcf_comparison.out.mendelian_vcf
        .map { meta, vcf -> [[id: meta.id], vcf] }
        .view { "DEBUG: Mendelian VCF: $it" }
