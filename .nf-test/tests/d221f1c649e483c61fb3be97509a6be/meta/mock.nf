...
include { CALL_STR } from '/home/nmahfel/nf-training/longraredisease/subworkflows/local/call_str/tests/../main.nf'
...
workflow {
    ...
    //run workflow
    CALL_STR.run(input.toArray())
    
    if (CALL_STR.output){

        // consumes all named output channels and stores items in a json file
        CALL_STR.out.getNames().each { name ->
            serializeChannel(name, CALL_STR.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  
    
        // consumes all unnamed output channels and stores items in a json file
        def array = CALL_STR.out as List<Object>
        def i = 0
        array.each { output ->
